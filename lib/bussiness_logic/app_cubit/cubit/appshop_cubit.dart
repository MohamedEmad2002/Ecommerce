...
class AppshopCubit extends Cubit<AppshopState> {
  ...
  void getHomedata() {
    DioHelper.getData(path: Endpoints.home, token: token).then((value) {
  ...
  void getUserData() {
    DioHelper.getData(path: Endpoints.profile, token: token).then((value) {
...
emit(Changefavourites());
  ...
  void getFavourites(){
        DioHelper.getData(path: Endpoints.favorites,
